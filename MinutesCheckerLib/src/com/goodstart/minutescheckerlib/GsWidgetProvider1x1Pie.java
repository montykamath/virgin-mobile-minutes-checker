...
public class GsWidgetProvider1x1Pie extends GsWidgetProvider
{
	public static void updateWidgets(GsAccountList accountList, Context context)
	{
		...
		ComponentName widgetComponent = new ComponentName(context, GsWidgetProvider1x1Pie.class);
		int[] widgetIds = widgetManager.getAppWidgetIds(widgetComponent);
		Intent intent = new Intent();
		intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS, widgetIds);
		intent.setAction("GsWidgetProvider1x1Pie_UPDATE");
	...
	void updateViews(Context context, AppWidgetManager appWidgetManager,int[] appWidgetIds, GsAccount account)
	{
	    ...
	    Bitmap bitmap = new GsPieChart().getPieChartBitmap(context, account, false);
