...
public class GsWidgetProvider1x1 extends GsWidgetProvider
{
	...
	public static void updateWidgets(GsAccountList accountList, Context context)
	{
		...
		ComponentName widgetComponent = new ComponentName(context, GsWidgetProvider1x1.class);
		int[] widgetIds = widgetManager.getAppWidgetIds(widgetComponent);
		Intent intent = new Intent();
		intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_IDS, widgetIds);
		intent.setAction("GsWidgetProvider1x1_UPDATE");
		context.sendBroadcast(intent);
	}
	
	void updateViews(Context context, AppWidgetManager appWidgetManager,int[] appWidgetIds, GsAccount account)
	{
		//Log.i(this.getClass().getSimpleName(), "About to update 1x1 widgets: " + Arrays.toString(appWidgetIds));
		
		RemoteViews remoteViews;
		if(account == null || account.getErrorMessage() != null)
		{
			remoteViews = new RemoteViews(context.getPackageName(), R.layout.widget_1x1_error);
		}
		else
		{
